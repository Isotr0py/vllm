...
# ---------------------------------------------------------------------------
# Inline utilities (originally from tile_kernels.utils)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Scoring functions (originally from tile_kernels.moe.scoring)
# ---------------------------------------------------------------------------


class _ScoringFunc(IntEnum):
    SIGMOID = 0
    SQRTSOFTPLUS = 1
    SOFTMAX = 2
    IDENTITY = 3
...
@T.macro
def _softplus(x: T.Ref):
    threshold = 20.0
    return T.if_then_else(x > threshold, x, T.log1p(T.exp(x)))


# ---------------------------------------------------------------------------
# Grouped top-k helpers (originally from tile_kernels.moe.common)
# ---------------------------------------------------------------------------


@T.macro
def _get_topk_group_idx(
    ...
    scores_vec_local = T.alloc_local(
        (num_vectorize_for_grouped_expert,), dtype=T.float32
    )
    ...
    if lane_idx < num_groups:
        num_vec_experts_per_group = (
            num_experts_per_group // num_vectorize_for_grouped_expert
        )
        for i in T.unroll(num_vec_experts_per_group):
            for j in T.vectorized(num_vectorize_for_grouped_expert):
                ...
                scores_vec_local[j] = scores_shared[
                    token_idx,
                    lane_idx * num_experts_per_group
                    + vec_idx * num_vectorize_for_grouped_expert
                    + j,
                ]
    ...
    for i in T.unroll(num_groups):
        ...
        if other_top2_sum > topk_sum_var or (
            other_top2_sum == topk_sum_var and i < lane_idx
        ):
            count_var += 1
...
def get_top2_sum_gate_kernel(
    ...
    num_vectorize = 4
    num_topk_sum = 2
    assert num_routed_experts % num_vectorize == 0, (
        f"`num_routed_experts` must be divisible by {num_vectorize}, but got {num_routed_experts}"
    )
    ...
    if num_groups == num_topk_groups:
        ...
        assert num_groups <= warp_size, (
            f"num_groups ({num_groups}) must be <= warp size ({warp_size}) for warp-level group ranking"
        )
        assert num_routed_experts <= warp_size * num_groups, (
            f"No more than {warp_size} experts per group is supported."
        )
    ...
    num_routed_experts_per_thread = max(
        _align(_ceil_div(num_routed_experts, warp_size), num_vectorize), num_topk_groups
    )
    ...
    def top2_sum_gate_kernel(
        ...
        to_physical_map: T.Tensor[
            (num_logical_experts, num_duplicate_experts), T.int32
        ],
        logical_count: T.Tensor[(num_logical_experts), T.int32],
        topk_idx: T.Tensor[(num_tokens, num_physical_topk), T.int64],
        unmapped_topk_idx: T.StridedTensor[
            (num_tokens, num_topk), (unmapped_topk_idx_stride, 1), T.int64
        ],
        ...
        with T.Kernel(num_tokens, threads=num_threads) as pid:
            ...
            scores_shared = T.alloc_shared(
                (num_tokens_per_block, num_routed_experts), dtype=T.float32
            )
            scores_wo_bias_shared = T.alloc_shared(
                (num_tokens_per_block, num_routed_experts), dtype=T.float32
            )

            bias_local = T.alloc_local(
                (num_routed_experts_per_thread,), dtype=T.float32
            )
            scores_local = T.alloc_local(
                (num_routed_experts_per_thread,), dtype=T.float32
            )
            idx_local = T.alloc_local((num_routed_experts_per_thread,), dtype=T.int32)
            topk_group_idx_shared = T.alloc_shared(
                (num_tokens_per_block, num_topk_groups), dtype=T.int32
            )
            ...
            for i in T.unroll(0, num_routed_experts_per_thread // num_vectorize):
                start_expert_idx = (
                    i * num_vectorize * warp_size + lane_idx * num_vectorize
                )
                if start_expert_idx < num_routed_experts:
                    for j in T.vectorized(num_vectorize):
                        scores_local[i * num_vectorize + j] = logits[
                            global_token_idx, start_expert_idx + j
                        ]
                        bias_local[i * num_vectorize + j] = bias[start_expert_idx + j]
                        if scoring_type == 2:  # SOFTMAX
                            scores_shared[token_idx, start_expert_idx + j] = (
                                scores_local[i * num_vectorize + j]
                            )
                    ...
                    for j in T.unroll(num_vectorize):
                        ...
                        T.device_assert(
                            is_finite, msg="top2_sum_gate input contains nan or inf!"
                        )
            ...
            if scoring_type == 2:  # SOFTMAX
                ...
                for i in T.unroll(
                    0, T.ceildiv(num_routed_experts, num_vectorize * warp_size)
                ):
                    if (
                        i * num_vectorize * warp_size + lane_idx * num_vectorize
                        < num_routed_experts
                        ...
                        for j in T.unroll(num_vectorize):
                            scores_local[i * num_vectorize + j] = T.exp(
                                scores_local[i * num_vectorize + j] - logit_max_var
                            )
                ...
                if start_expert_idx < num_routed_experts:
                    for j in T.vectorized(num_vectorize):
                        ...
                        if scoring_type == 0:  # SIGMOID
                            ...
                            scores_local[local_idx] = T.sqrt(
                                _softplus(scores_local[local_idx])
                            )
                        elif scoring_type == 2:  # SOFTMAX
                            scores_local[local_idx] = (
                                scores_local[local_idx] / logit_sum_var
                            )
                        ...
                        scores_wo_bias_shared[token_idx, expert_idx] = scores_local[
                            local_idx
                        ]

                        if scoring_type == 2:  # SOFTMAX
                            scores_local[local_idx] = scores_shared[
                                token_idx, expert_idx
                            ]
                        scores_local[local_idx] += bias_local[local_idx]
                        if not skip_group_sort:
                            scores_shared[token_idx, expert_idx] = scores_local[
                                local_idx
                            ]
                        else:
            ...
            if not fix_routing_mask_exists or not fix_routing_mask[global_token_idx]:
                ...
                if not skip_group_sort:
                    ...
                    for i in T.unroll(num_topk_groups):
                        for j in T.unroll(num_topk_groups):
                            if (
                                j > i
                                and topk_group_idx_local[j] < topk_group_idx_local[i]
                            ):
                    ...
                    if lane_idx < num_routed_experts_per_group:
                        for i in T.unroll(num_topk_groups):
                            ...
                            scores_local[i] = scores_shared[
                                token_idx,
                                select_group_idx * num_routed_experts_per_group
                                + lane_idx,
                            ]
                            idx_local[i] = (
                                select_group_idx * num_routed_experts_per_group
                ...
                for k in T.unroll(num_topk):
                    ...
                    for i in T.unroll(5):
                        ...
                        if other_score > topk_scores_local[k] or (
                            other_score == topk_scores_local[k]
                            and other_idx < topk_idx_local[k]
                        ):
            ...
            if to_physical_map_exists and lane_idx < num_physical_topk:
                ...
                duplicate_idx = (
                    ep_rank + global_token_idx * large_prime_number
                ) % num_duplicates
                topk_idx_var = to_physical_map[logical_expert_idx, duplicate_idx]

            # Mask ETP idx
            num_experts_per_rank = (
                num_routed_experts + num_extra_experts
            ) // num_ep_ranks
            ...
            if lane_idx < num_physical_topk:
                ...
                if dst_ep_rank % num_tp_ranks != T.int64(tp_rank):
                    ...
                    topk_idx_var = (
                        topk_idx_var
                        - dst_dp_rank * num_experts_per_dp
                        + dst_dp_rank * num_experts_per_rank
                    )
...
def top2_sum_gate(
    ...
    """
    assert (
        logits.dim() == 2 and logits.is_contiguous() and logits.dtype == torch.float32
    )
    ...
    assert (num_groups == 0) == (num_topk_groups == 0), (
        "num_groups and num_topk_groups must be both zero or both non-zero."
    )
    assert num_topk <= num_routed_experts, (
        f"num_topk ({num_topk}) must be less than or equal to num_routed_experts ({num_routed_experts})."
    )
    assert num_topk_groups <= num_groups, (
        f"num_topk_groups ({num_topk_groups}) must be less than or equal to num_groups ({num_groups})."
    )
    if num_groups != 0:
        assert num_routed_experts % num_groups == 0, (
            f"num_routed_experts ({num_routed_experts}) must be divisible by num_groups ({num_groups})."
        )

    assert _ScoringFunc.from_str(scoring_func) != _ScoringFunc.IDENTITY, (
        "IDENTITY scoring function is not currently supported for top-2 sum moe."
    )
    ...
    assert num_shared_experts in {1, 2} or (
        num_shared_experts == 0 and not use_shared_as_routed
    )

    topk_idx = torch.empty(
        num_tokens,
        num_topk + num_shared_experts,
        dtype=torch.long,
    ...
    if unmapped_topk_idx is not None:
        ...
        assert (
            unmapped_topk_idx.size(0) == num_tokens
            and unmapped_topk_idx.size(1) == num_topk
        )
    ...
    kernel = get_top2_sum_gate_kernel(
        _ScoringFunc.from_str(scoring_func).value,
        num_topk,
