...
class WeightsMapper:
    ...
    orig_to_new_stacked: Mapping[str, str | tuple[str, str | int] | None] = field(
    ...
    def _map_name(self, key: str) -> str | None:
        """Map a weight name (backward-compatible wrapper that discards
        shard_id)."""
        result = self._map_name_with_shard(key)
        return result[0] if result is not None else None

    def _map_name_with_shard(self, key: str) -> tuple[str, str | int | None] | None:
        ...
        for substr, new_value in self.orig_to_new_stacked.items():
            if substr in key:
                if new_value is None:
                    return None
                if isinstance(new_value, tuple):
                    new_key, shard_id = new_value
                else:
                    new_key = new_value
