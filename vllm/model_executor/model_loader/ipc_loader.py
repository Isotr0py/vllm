...
server via CUDA IPC instead of loading from disk."""
...
from vllm.model_executor.model_loader.weight_cache.protocol import (
    ...
    WeightsCacheFingerprint,
    WeightsCacheMismatchError,
...
class IpcModelLoader(BaseModelLoader):
    """Loads a model by mapping the weight cache server's tensors via CUDA IPC.

    The model is initialized on the meta device and every parameter/buffer is
    replaced by the server's post-quantized tensor, so
    process_weights_after_loading is skipped entirely. In "zero_copy" mode the
    engine shares the server's GPU memory; in "copy" mode the tensors are
    cloned into engine-owned memory and the server is asked to release its
    cache afterwards.

    Extra config keys (via --model-loader-extra-config):

    - socket_path: explicit server socket path. Defaults to a per-GPU path
      derived from the physical GPU id.
    - socket_dir: directory containing the server sockets.
    - mode: "zero_copy" (default) or "copy".
    - fallback: fall back to disk loading when the server is unavailable or
    ...
    Note: in zero-copy mode the weights live in the server's CUDA IPC
    ...
    def load_weights(self, model: nn.Module, model_config: ModelConfig) -> None:
        ...
        Copies server tensors into matching parameters/buffers. The model is
    ...
    def load_model(
        ...
        # transient server outage, so it is raised even when fallback is on.
        ...
        except (WeightCacheUnavailableError, WeightsCacheMismatchError) as e:
    ...
    def _build_model(
        ...
        if self.mode == "copy":
            self._send_release()
        logger.info(
            "Mapped %d tensors from the weight cache server (%s mode)",
            len(entries),
            self.mode,
        )
        return model.eval()
    ...
    def _check_supported(vllm_config: VllmConfig, model_config: ModelConfig) -> None:
        ...
        if cache_dtype != "auto":
            # Quantized kv cache dtypes keep host-side scale state that
            # process_weights_after_loading derives from the checkpoint;
            # tensor export cannot carry it, so they are not supported.
    ...
    def _fetch_entries(
        ...
        cache_config = WeightsCacheFingerprint.from_model_config(
            model_config,
    ...
    def _request_state(
        self, cache_config: WeightsCacheFingerprint
    ) -> tuple[dict[str, TensorEntry], dict[str, str]]:
        ...
        if status == "mismatch":
            raise WeightsCacheMismatchError(
                f"WeightsCacheFingerprint mismatch on fields: {response.get('fields')}"
            )
        if status != "ok":
            raise WeightCacheUnavailableError(
                f"Weight cache server error: {response.get('message')}"
            )
    ...
    def _connect(self, timeout: float) -> socket.socket:
        ...
        try:
            ...
            sock.close()
            raise WeightCacheUnavailableError(
                f"Cannot connect to weight cache server at {socket_path}: {e}"
            ) from e
        return sock
    ...
    def _check_gpu_uuid(self, server_uuid: str | None) -> None:
        ...
        if server_uuid != local_uuid:
            raise WeightsCacheMismatchError(
                f"Server GPU {server_uuid} != engine GPU {local_uuid}; "
    ...
    def _send_release(self) -> None:
        try:
            ...
            logger.warning("Failed to ask the weight cache server to release")
...
def _materialize_remaining_meta_tensors(model: nn.Module, device: torch.device) -> None:
    """Allocate any tensors the server did not provide.

    These are typically parameters removed on the server side by
