...
"""Fingerprinting and socket protocol for the weight cache server.
...
sockets live in a per-user private directory (mode 0700) and the server
restricts the socket file permissions to the owner (0600). Both the server and
...
class WeightCacheUnavailableError(Exception):
    """Raised when no weight cache server is reachable or usable."""


class WeightsCacheMismatchError(Exception):
    """Raised when the server's cached weights don't match the engine."""
...
def check_ipc_quant_support(model_config: ModelConfig, *, where: str) -> None:
    ...
    Args:
        ...
        where: Short tag ("server"/"engine") used in the error message.
    ...
    raise UnsupportedQuantForIPCError(
        ...
        f"wrong numerics. Verified: {verified}. Use the default "
        f"--load-format for this model."
    )
...
def get_socket_dir(socket_dir: str | None = None) -> str:
    """Return the directory that holds the server sockets.
...
def ensure_private_socket_dir(directory: str, strict_perms: bool = True) -> None:
    ...
    Called by the server before binding. Existing directories are re-checked
...
class WeightsCacheFingerprint:
    """Fingerprint of the cached weights.

    Any mismatch between the server's and the engine's fingerprint means the
    ...
    def from_model_config(
        ...
        between the server and the engine.
        """
    ...
    def mismatched_fields(self, other: "WeightsCacheFingerprint") -> list[str]:
        return [
            f.name
...
class TensorEntry:
    ...
    def rebuild(self, device_index: int) -> torch.Tensor:
        ...
        # be retargeted to the local index since the server and the engine may
