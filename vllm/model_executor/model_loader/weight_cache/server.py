...
"""Weight cache server for fast engine restarts.

One server process per GPU holds the post-quantized, TP-sharded weights of its
...
Launch one server per TP rank with a single command:

    python -m vllm.model_executor.model_loader.weight_cache.server \\
        --model /path/to/model --tensor-parallel-size 4

Engines then load from the servers with:
...
def _report_ready(message: str) -> None:
    ...
    afterwards -- including the server's readiness announcement. Since operators
    rely on that line to know the server is serving, write it directly to file
...
class WeightCacheServer:
    ...
    def __init__(
        ...
        self.cache_config = WeightsCacheFingerprint.from_model_config(
    ...
    def load_model(self) -> None:
        ...
        logger.info(
            "Weight cache server rank %d cached %d tensors",
            self.tp_rank,
            len(self.entries),
        )
    ...
    def serve_forever(self, ready_callback: Callable[[], None] | None = None) -> None:
        ...
        get a connection error (and fall back to disk) until the server is
        ready.
        ...
        server.listen()
        logger.info(
            "Weight cache server rank %d serving on %s", self.tp_rank, socket_path
        )
        print(
            f"Weight cache server rank {self.tp_rank} ready: serving on {socket_path}"
        )
            ...
            while True:
                ...
                with conn:
                    try:
                        ...
                        # down the server for every other engine on this GPU.
                        logger.exception(
    ...
    def _handle_get_state(self, conn: socket.socket, request: dict) -> None:
        ...
        if not isinstance(client_config, WeightsCacheFingerprint):
            send_msg(conn, {"status": "error", "message": "Missing cache_config"})
            return
        mismatched = self.cache_config.mismatched_fields(client_config)
        if mismatched:
            logger.warning("WeightsCacheFingerprint mismatch on fields: %s", mismatched)
    ...
    def _handle_release(self, conn: socket.socket) -> None:
        ...
        logger.info("Weight cache server rank %d released cached weights", self.tp_rank)
...
def _run_server(
    ...
    server = WeightCacheServer(
        vllm_config, tp_rank, distributed_init_method, socket_dir
    )
    server.load_model()
    server.serve_forever(ready_callback=lambda: ready_queue.put(tp_rank))
...
def main() -> None:
    ...
    parser = FlexibleArgumentParser(
        description="Launch weight cache servers (one per TP rank)."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for the server Unix sockets (default: tempdir).",
    )
    ...
    if vllm_config.load_config.load_format == "ipc_cache":
        raise ValueError(
            "The weight cache server itself must load from disk; use the "
            "default --load-format"
        )
    ...
    check_ipc_quant_support(vllm_config.model_config, where="server")
    parallel_config = vllm_config.parallel_config
    if parallel_config.pipeline_parallel_size > 1:
        raise ValueError(
            "The weight cache server only supports tensor parallelism; "
    ...
    procs = [
        ctx.Process(
            ...
            name=f"vllm-weight-cache-server-{rank}",
        )
    ...
    while len(ready_ranks) < tp_size:
        try:
            ...
            if dead:
                logger.error(
                    "Weight cache server rank(s) exited during startup "
    ...
    logger.info(
        "===== Weight cache server READY: all %d ranks serving in %s =====",
        tp_size,
        args.weight_cache_socket_dir or "the default socket dir",
    )
    _report_ready(
        f"===== Weight cache server READY: all {tp_size} rank(s) serving in "
