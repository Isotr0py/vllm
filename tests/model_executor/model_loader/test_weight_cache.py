...
"""Tests for the weight cache server protocol and the IPC model loader."""
...
def _make_cache_config(**overrides) -> WeightsCacheFingerprint:
    kwargs = dict(
        model="test-model",
    ...
    return WeightsCacheFingerprint(**kwargs)
...
def test_unavailable_server_raises(tmp_path):
    loader = IpcModelLoader(
        LoadConfig(
            load_format="ipc_cache",
...
def _request_against_fake_server(tmp_path, response: dict):
...
def test_mismatch_response_raises(tmp_path):
    with pytest.raises(WeightsCacheMismatchError, match="tp_rank"):
        _request_against_fake_server(
...
def test_ok_response_returns_entries(tmp_path):
    ...
    entries, aliases = _request_against_fake_server(
...
def test_export_entries_records_tied_alias():
    from vllm.model_executor.model_loader.weight_cache.server import export_entries
...
@pytest.mark.parametrize(
    ...
    [
        (None, None, True),
        ("fp8", {"weight_block_size": [128, 128]}, False),
        ("awq", {}, False),
...
def test_tensor_entry_cross_process_ipc():
    ...
    try:
        ...
        tensor = entry.rebuild(torch.accelerator.current_device_index())
