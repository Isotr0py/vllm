...
def benchmark_run(
    ...
    preprocessor = engine.input_preprocessor


    def _chat(idx: int, item):
        preprocessor.preprocess(prompt)
...
async def benchmark_run_async(
    ...
    preprocessor = await engine.get_input_preprocessor()

    async def _chat(idx: int, item):
        await preprocessor.preprocess_async(prompt={
            "prompt": prompt,
            "multi_modal_data": {
                modality: item
            }
